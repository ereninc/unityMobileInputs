...
{
    [SerializeField] private Text _multiTouchInfoText;
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0)
        {
            for (int i = 0; i < Input.touchCount; i++)
            {
                ...
                _multiTouchInfo += string.Format("Touch {0} - Position {1} - Tap Count {2} - Finger ID {3}\nRadius {4} ({5}%)\n",
        ...
        _multiTouchInfoText.text = _multiTouchInfo;
    }
}